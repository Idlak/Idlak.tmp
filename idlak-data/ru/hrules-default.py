...
def numbers_ru(norm, string, args):
    # set defaults arguments
    #print (args)
    setdefaultarg(args, 'separator', ',')
    setdefaultarg(args, 'leadingzeros', 'f')
    ...
    if string == '':
        return ''
    # search for parent lookup, if not found use digitlkp
    parentlkp = lookup(norm.lkps[args['case']], 'parent')
    if not parentlkp or parentlkp not in norm.lkps:
        parentlkp = 'digitlkp'
    number_recursive_ru(norm.lkps[parentlkp], norm.lkps[args['case']], string, args, output)
    return ' '.join(output)

def number_recursive_ru(lkp, lkpcase, string, args, output):
    # check if whole number is present
    if lookup(lkpcase, string):
        output.append(lookup(lkpcase, string))
        return
    ...
    if n >= 1000000000:
        factor = n / 1000000000
        final_digit = factor % 10
        if (final_digit > 1 and final_digit < 5) and (factor < 12 or factor > 15):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000000,
                               'billion', 'billion_plural_2_4', 'billion_join')
        elif (final_digit == 0 or (final_digit >= 5 and final_digit <= 9) or
                (factor >= 10 and factor <= 14)):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000000,
                               'billion', 'billion_plural', 'billion_join')
        else:
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000000,
                               'billion', 'billion', 'billion_join')

    if n >= 1000000:
        factor = n / 1000000
        final_digit = factor % 10
        if (final_digit > 1 and final_digit < 5) and (factor < 12 or factor > 15):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000,
                               'million', 'million_plural_2_4', 'million_join')
        elif (final_digit == 0 or (final_digit >= 5 and final_digit <= 9) or
                (factor >= 10 and factor <= 14)):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000,
                               'million', 'million_plural', 'million_join')
        else:
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000000,
                               'million', 'million', 'million_join')

    if n >= 1000:
        factor = n / 1000
        final_digit = factor % 10
        if (final_digit > 1 and final_digit < 5) and (factor < 12 or factor > 15):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000,
                               'thousand', 'thousand_plural_2_4', 'thousand_join')
        elif (final_digit == 0 or (final_digit >= 5 and final_digit <= 9) or
                (factor >= 10 and factor <= 14)):
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000,
                               'thousand', 'thousand_plural', 'thousand_join')
        else:
            n = numberchunk_ru(lkp, lkpcase, n, args, output, 1000,
                               'thousand', 'thousand', 'thousand_join')

    if n >= 100:
        n = numberchunk_ru(lkp, lkpcase, n, args, output, 100,
                           'hundred', 'hundred_plural_2_4', 'hundred_join')
...
def numberchunk_ru(lkp, lkpcase, n, args, output, scale, scalename_sing, scalename_pl, join_name):
    scalen = int(n / scale);
    remainder = n % scale
    checkn = scalen * scale
    if scalen < 1:
        return n
    # check if the number without remainder is present
    if lookup(lkpcase, str(checkn)):
        output.append(lookup(lkpcase, str(checkn)))
        return remainder
    # check if whole number is present
    if lookup(lkpcase, str(n)):
        output.append(lookup(lkpcase, str(n)))
    ...
    if scalen > 1:
        if remainder:
            output.append(lookup(lkp, scalename_pl))
        else:
            output.append(lookup(lkpcase, scalename_pl))
    else:
        if remainder:
            output.append(lookup(lkp, scalename_sing))
        else:
            output.append(lookup(lkpcase, scalename_sing))
    if remainder:
        # add 'and' as required
        if lookup(lkp, join_name):
            output.append(lookup(lkp, join_name))
    return remainder
...
def aschars(norm, string, args):
    ...
    for c in string:
        ...
        elif 'currency' in norm.lkps and c in norm.lkps['currency']:
...
NORMFUNCS = {'asdigits': asdigits, 'number':number, 'roman':roman, 'aschars': aschars, 'filter': filter, 'numbers_ru': numbers_ru}
